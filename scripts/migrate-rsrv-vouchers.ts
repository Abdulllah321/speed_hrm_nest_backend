...
async function migrateRsrvVouchers(prisma: any, dbName: string, isDryRun: boolean = false) {
  ...
  for (const rv of rsrvVouchers) {
    ...
    // Only migrate if there is an explicit "RSRV # XXX" typed in remarks/description
    // or if the voucher was already created as RS-RV / RSRV.
    // Skip autogenerated standard receipt vouchers that don't have explicit RSRV numbers.
  ...
  if (isDryRun) {
    ...
    console.log(`   Matched & Would Migrate: ${updatedCount} voucher(s) (with explicit RSRV # in remarks)`);
