...
function getAccountCodes(nature) {
    switch (nature) {
...
function getImportAccountCodes(category) {
    switch (category) {
...
async function seedVendors(prisma) {
    ...
    for (const v of allVendors) {
        ...
        const existing = await prisma.vendor?.findFirst?.({ where: { code: v.code } })
            ?? await prisma.supplier?.findFirst?.({ where: { code: v.code } });
        const data = {
            code: v.code,
            name: v.name,
            brand: v.brand ?? null,
            type: v.code.startsWith('IMP') ? 'IMPORT' : 'LOCAL',
            nature: v.nature === 'GOODS / SERVICES' ? 'GOODS' : v.nature,
            address: v.address,
        ...
        try {
            const model = prisma.vendor ?? prisma.supplier;
            if (!model) {
