...
export class IntegrationService {
    ...
    async provisionTenant(dto: ProvisionTenantDto) {
        ...
        if (tenant) {
            ...
            this.logger.log(`Provisioning new company for dealer: ${externalId} with code: ${tenantCode}`);
    ...
    async provisionUser(dto: ProvisionUserDto) {
        const { externalId, dealerId, email, firstName, lastName, role, isActive = true } = dto;
