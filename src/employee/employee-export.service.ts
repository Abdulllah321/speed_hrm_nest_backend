...
export interface QueueEmployeeExportOptions {
  userId: string;
  search?: string;
  departmentId?: string;
...
export class EmployeeExportService {
  ...
  constructor(
    @InjectQueue('employee-export') private readonly exportQueue: Queue,
    private readonly prisma: PrismaService,
    private readonly uploadService: UploadService,
  ) {}

  async queueExport(opts: QueueEmployeeExportOptions): Promise<{ jobId: string }> {
    const jobId = uuidv4();
    ...
    await this.prisma.exportHistory.create({
      data: {
        id: jobId,
        userId: opts.userId,
        fileName: `employees-export-${new Date().toISOString().slice(0, 10)}.xlsx`,
        filePath: path.join('uploads', 'exports', `export-${jobId}.xlsx`),
        moduleName: 'EMPLOYEE_EXPORT',
        status: 'PENDING',
      },
    });
    ...
    this.logger.log(`[EmployeeExport] Queued job ${jobId} for user ${opts.userId} (tenant: ${tenantId})`);
    return { jobId };
  }
  ...
  async streamExportFile(jobId: string, res: any): Promise<void> {
    ...
    stream.on('close', () => {
      fs.unlink(filePath, (err) => {
        ...
        else     this.logger.log(`[EmployeeExport] Cleaned up ${filePath}`);
      });
    });
    stream.on('error', (err) => {
      this.logger.error(`[EmployeeExport] Stream error: ${err.message}`);
    });
