...
export interface QueueLoanRequestExportOptions {
  userId: string;
}

@Injectable()
export class LoanRequestExportService {
  private readonly logger = new Logger(LoanRequestExportService.name);

  constructor(
    @InjectQueue('loan-request-export') private readonly exportQueue: Queue,
  ...
  async queueExport(opts: QueueLoanRequestExportOptions): Promise<{ jobId: string }> {
    ...
    await this.prisma.exportHistory.create({
      data: {
        id: jobId,
        userId: opts.userId,
        fileName: `employee-loan-account-${new Date().toISOString().slice(0, 10)}.xlsx`,
        filePath: path.join('uploads', 'exports', `export-${jobId}.xlsx`),
        moduleName: 'LOAN_REQUEST_EXPORT',
        status: 'PENDING',
      },
    });
