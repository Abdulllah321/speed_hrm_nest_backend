...
export class LocationService {
    ...
    async update(id: string, body: { name: string; address?: string; cityId?: string; status?: string }, ctx: { userId?: string; ipAddress?: string; userAgent?: string }) {
        try {
            ...
            return { status: false, message: error instanceof Error ? error.message : 'Failed to update location' }
        }
    }
    ...
    async createBulk(
        ...
        if (!items?.length) return { status: false, message: 'No locations to create' }
        try {
    ...
    async updateBulk(
        ...
        if (!items?.length) return { status: false, message: 'No locations to update' }
        try {
            for (const i of items) {
                const existing = await this.prisma.location.findUnique({ where: { id: i.id } })
    ...
    async removeBulk(ids: string[], ctx: { userId?: string; ipAddress?: string; userAgent?: string }) {
        if (!ids?.length) return { status: false, message: 'No locations to delete' }
        try {
            const existing = await this.prisma.location.findMany({ where: { id: { in: ids } } })
            const result = await this.prisma.location.deleteMany({ where: { id: { in: ids } } })
            await this.activityLogs.log({
                userId: ctx.userId,
                action: 'delete',
                module: 'locations',
