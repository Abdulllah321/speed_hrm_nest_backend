...
export class PurchaseRequisitionService {
    ...
    async update(id: string, updateDto: UpdatePurchaseRequisitionDto) {
        ...
        if (updateDto.status && (updateDto.status === 'APPROVED' || updateDto.status === 'REJECTED')) {
            if (pr.status !== 'SUBMITTED') {
                throw new BadRequestException('Only SUBMITTED requisitions can be approved or rejected');
            }
        }
        ...
        if (items) {
            if (pr.status !== 'DRAFT') throw new BadRequestException('Cannot modify items unless in DRAFT status');
