...
export class VendorQuotationService {
    ...
    async create(createDto: CreateVendorQuotationDto) {
        ...
        const isVendorInRfq = rfq.vendors.some(v => v.vendorId === createDto.vendorId);
        if (!isVendorInRfq) {
    ...
    async update(id: string, updateDto: UpdateVendorQuotationDto) {
        ...
        if (items) {
            if (quotation.status !== 'DRAFT') {
                throw new BadRequestException('Cannot modify items unless in DRAFT status');
            }

            return this.prisma.$transaction(async (tx) => {
                await tx.vendorQuotationItem.deleteMany({ where: { vendorQuotationId: id } });
    ...
    private calculateLineTotal(qty: number, price: number, taxPercent: number, discountPercent: number): number {
