...
export class RfqService {
    ...
    async create(createDto: CreateRfqDto) {
        ...
        if (pr.status !== 'APPROVED') {
            throw new BadRequestException('Only APPROVED Purchase Requisitions can be converted to RFQ');
        }
    ...
    async addVendors(id: string, addVendorsDto: AddVendorsDto) {
        ...
        const newVendorIds = addVendorsDto.vendorIds.filter(id => !existingVendorIds.includes(id));

        if (newVendorIds.length === 0) {
            throw new BadRequestException('All vendors are already added to this RFQ');
        }
