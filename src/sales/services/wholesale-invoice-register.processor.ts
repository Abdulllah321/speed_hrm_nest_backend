...
export interface WholesaleInvoicePreviewJobData {
  jobId: string;
  userId: string;
  tenantId: string;
  tenantDbUrl: string;
...
export class WholesaleInvoiceRegisterProcessor {
  private readonly logger = new Logger(WholesaleInvoiceRegisterProcessor.name);

  constructor(
    private readonly wholesaleInvoiceService: WholesaleInvoiceRegisterService,
  ...
  @Process('export-wholesale-invoice-register-report')
  async handleExport(job: Job<WholesaleInvoiceExportJobData>): Promise<void> {
    const {
      jobId,
      userId,
      tenantId,
      ...
      async () => {
        ...
        try {
          ...
          await this.notificationsService.create({
            ...
            category: 'export',
            priority: 'high',
            actionType: `wholesale-invoice-export.ready`,
