...
@Injectable()
export class CustomerBulkUploadService {
    private readonly logger = new Logger(CustomerBulkUploadService.name);

    constructor(
        @InjectQueue('customer-upload') private uploadQueue: Queue,
    ...
    async initiateValidation(fileBuffer: Buffer, filename: string, userId: string): Promise<{ uploadId: string; jobId: string }> {
        ...
        const uploadDir = path.join(process.cwd(), 'uploads', 'bulk', 'customer');
        if (!fs.existsSync(uploadDir)) fs.mkdirSync(uploadDir, { recursive: true });

        const ext = filename.split('.').pop();
        const filePath = path.join(uploadDir, `customer-upload-${upload.id}.${ext}`);
    ...
    async confirmUpload(uploadId: string, userId: string): Promise<{ uploadId: string; jobId: string }> {
        ...
        if (upload.status !== 'validated') {
            throw new Error(`Upload must be 'validated' to confirm (current: ${upload.status})`);
        }
    ...
    async getUploadHistory(userId: string, limit = 50) {
        return this.prisma.bulkUpload.findMany({
            ...
            take: limit,
            select: { id: true, filename: true, status: true, totalRecords: true, successRecords: true, failedRecords: true, skippedRecords: true, createdAt: true, completedAt: true },
        });
    }
