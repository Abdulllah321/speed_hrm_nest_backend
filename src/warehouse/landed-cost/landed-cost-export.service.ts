...
export interface QueueLandedCostExportOptions {
  userId: string;
  landedCostId: string;
  search?: string;
  hsCodes?: string[];
...
@Injectable()
export class LandedCostExportService {
  private readonly logger = new Logger(LandedCostExportService.name);

  constructor(
    @InjectQueue('landed-cost-export') private readonly exportQueue: Queue,
