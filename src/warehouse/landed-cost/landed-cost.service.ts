...
export class LandedCostService {
  ...
  async create(dto: CreateLandedCostDto, ctx?: { userId?: string; ipAddress?: string; userAgent?: string }) {
    try {
      ...
      // 1) Resolve items to UUIDs and normalize inbound rates
      const resolvedItems = await Promise.all(
        dto.items.map(async (item) => {
          ...
          if (!itemRecord) {
            throw new BadRequestException(
              `Item with ID or code ${item.itemId} not found in master database`,
            );
          }

          const normalizedUnitCostPKR = this.resolveInboundUnitRate({
            qty: item.qty,
            unitCostPKR: item.unitCostPKR,
            totalCostPKR: item.totalCostPKR,
            unitPrice: item.unitFob,
          });

          return {
            ...item,
            itemId: itemRecord.id, // Use the proper UUID
            normalizedUnitCostPKR,
          };
        }),
      );
      ...
      runInBackground(
        ...
        this.activityLogs.log({
          ...
          description: `Created landed cost ${landedCost.landedCostNumber}`,
          newValues: JSON.stringify({ grnId: dto.grnId }),
      ...
      runInBackground(
        'Create Landed Cost (Failure)',
        this.activityLogs.log({
          userId: ctx?.userId,
          action: 'create',
          module: 'landed-cost',
          entity: 'LandedCost',
          description: `Failed to create landed cost for GRN ${dto.grnId}`,
  ...
  async createLocal(dto: CreateLandedCostDto, ctx?: { userId?: string; ipAddress?: string; userAgent?: string }) {
    try {
      const grn = await this.prisma.goodsReceiptNote.findUnique({
        where: { id: dto.grnId },
        include: {
      ...
      runInBackground(
        ...
        this.activityLogs.log({
          ...
          entity: 'LandedCost',
          entityId: landedCost.id,
          description: `Created local landed cost ${landedCost.landedCostNumber}`,
          newValues: JSON.stringify({ grnId: dto.grnId }),
          ipAddress: ctx?.ipAddress,
          userAgent: ctx?.userAgent,
          status: 'success',
      ...
      runInBackground(
        'Create Local Landed Cost (Failure)',
        this.activityLogs.log({
          userId: ctx?.userId,
          action: 'create',
          module: 'landed-cost',
          entity: 'LandedCost',
          description: `Failed to create local landed cost for GRN ${dto.grnId}`,
          errorMessage: error?.message,
          newValues: JSON.stringify({ grnId: dto.grnId }),
          ipAddress: ctx?.ipAddress,
          userAgent: ctx?.userAgent,
  ...
  async post(dto: { grnId: string; charges: { accountId: string; amount: number }[] }, ctx?: { userId?: string; ipAddress?: string; userAgent?: string }) {
    try {
      const grn = await this.prisma.goodsReceiptNote.findUnique({
        where: { id: dto.grnId },
        include: { items: true },
      ...
      runInBackground(
        ...
        this.activityLogs.log({
          ...
          module: 'landed-cost',
          entity: 'LandedCost',
          description: `Posted landed cost for GRN ${dto.grnId}`,
      ...
      runInBackground(
        'Post Landed Cost (Failure)',
        this.activityLogs.log({
          userId: ctx?.userId,
          action: 'create',
          module: 'landed-cost',
          entity: 'LandedCost',
          description: `Failed to post landed cost for GRN ${dto.grnId}`,
          errorMessage: error?.message,
          newValues: JSON.stringify(dto),
          ipAddress: ctx?.ipAddress,
          userAgent: ctx?.userAgent,
