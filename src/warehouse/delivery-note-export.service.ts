...
export class DeliveryNoteExportService {
  ...
  async queueExport(opts: QueueDeliveryNoteExportOptions): Promise<{ jobId: string }> {
    ...
    this.logger.log(`[DeliveryNoteExport] Queued job ${jobId} for user ${opts.userId} (tenant: ${tenantId})`);
    return { jobId };
  }
  ...
  async streamExportFile(jobId: string, res: any): Promise<void> {
    ...
    const filename  = `delivery-notes-export-${timestamp}.xlsx`;

    const stream = fs.createReadStream(filePath);
    stream.on('close', () => {
      fs.unlink(filePath, (err) => {
        if (err) this.logger.warn(`Could not delete export file: ${err.message}`);
        else     this.logger.log(`[DeliveryNoteExport] Cleaned up ${filePath}`);
      });
    });
    stream.on('error', (err) => {
      this.logger.error(`[DeliveryNoteExport] Stream error: ${err.message}`);
    });
