...
export class HolidayService {
  ...
  async create(
    ...
    try {
      ...
      if (error?.code === 'P2002') {
        return {
          status: false,
          message: 'A holiday with this name already exists',
        };
      }
  ...
  async createBulk(
    ...
    if (!items?.length)
      return { status: false, message: 'No holidays to create' };
  ...
  async updateBulk(
    ...
    if (!items?.length)
      return { status: false, message: 'No holidays to update' };
    ...
    try {
      for (const item of items) {
        ...
        if (item.dateFrom !== undefined) {
          updateData.dateFrom = this.normalizeToBaseYear(
