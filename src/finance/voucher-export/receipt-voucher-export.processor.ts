...
export interface RvExportJobData {
  jobId: string;
  userId: string;
  tenantId: string;
  tenantDbUrl: string;
...
export class ReceiptVoucherExportProcessor {
  private readonly logger = new Logger(ReceiptVoucherExportProcessor.name);
  ...
  async handleExport(job: Job<RvExportJobData>): Promise<void> {
    ...
    try {
      ...
      const total = await (prisma as any).receiptVoucher.count({ where });
      this.logger.log(`[RvExport ${jobId}] ${total} vouchers to export`);
      ...
      const ws = workbook.addWorksheet('Receipt Vouchers', {
      ...
      while (true) {
        ...
        for (const rv of chunk) {
          const details = rv.details.length > 0 ? rv.details : [null];
          ...
          details.forEach((detail: any, dIdx: number) => {
            ...
            const rowData: Record<string, any> = {
              rvNo:             dIdx === 0 ? rv.rvNo : '',
              rvDate:           dIdx === 0 ? new Date(rv.rvDate) : null,
              ...
              description:      dIdx === 0 ? (rv.description ?? '') : '',
      ...
      titleRow.getCell(1).value     = 'Receipt Voucher Export Summary';
      ...
      this.logger.log(`[RvExport ${jobId}] File written (${processedVouchers} vouchers, ${rowIdx} rows)`);

      await this.notificationsService.create({
        userId,
        title: 'Receipt Voucher Export Ready',
        message: `Your export of ${processedVouchers.toLocaleString()} receipt voucher${processedVouchers !== 1 ? 's' : ''} is ready to download.`,
        category: 'export',
        priority: 'high',
        actionType: 'receipt-voucher-export.ready',
        actionPayload: { jobId },
        entityType: 'receipt-voucher-export',
        entityId: jobId,
        channels: ['inApp'],
      });

    } catch (error: any) {
      this.logger.error(`[RvExport ${jobId}] FAILED: ${error.message}`, error.stack);
      if (fs.existsSync(filePath)) fs.unlinkSync(filePath);

      await this.notificationsService.create({
        userId,
        title: 'Receipt Voucher Export Failed',
