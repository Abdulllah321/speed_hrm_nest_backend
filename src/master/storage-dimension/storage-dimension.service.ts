...
export class StorageDimensionService {
    ...
    async create(createDto: CreateStorageDimensionDto, userId: string) {
        ...
        return { status: true, data: result, message: 'Storage Dimension created successfully' };
    }
    ...
    async findOne(id: string) {
        const storageDimension = await this.prismaMaster.storageDimension.findUnique({
            where: { id },
        });
        if (!storageDimension) {
    ...
    async update(id: string, updateDto: UpdateStorageDimensionDto) {
        ...
        return { status: true, data: result, message: 'Storage Dimension updated successfully' };
    }
