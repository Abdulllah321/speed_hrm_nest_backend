...
export class EquipmentService {
  ...
  async list() {
    const items = await this.prismaMaster.equipment.findMany({
  ...
  async get(id: string) {
    const item = await this.prismaMaster.equipment.findUnique({
      where: { id },
    });
  ...
  async create(
    ...
    try {
      const created = await this.prismaMaster.equipment.create({
        data: {
          name: body.name,
  ...
  async update(
    ...
    try {
      const existing = await this.prismaMaster.equipment.findUnique({
        where: { id },
      });
      const updated = await this.prismaMaster.equipment.update({
        where: { id },
        data: {
  ...
  async remove(
    ...
    try {
      ...
      const removed = await this.prismaMaster.equipment.delete({
        where: { id },
      });
      await this.activityLogs.log({
        userId: ctx.userId,
  ...
  async createBulk(
    ...
    try {
      const result = await this.prismaMaster.equipment.createMany({
        data: items.map((i) => ({
          name: i.name,
  ...
  async updateBulk(
    ...
    try {
      for (const i of items) {
        ...
        await this.prismaMaster.equipment.update({
          where: { id: i.id },
          data: {
  ...
  async removeBulk(
    ...
    try {
      const existing = await this.prismaMaster.equipment.findMany({
        where: { id: { in: ids } },
      });
      const result = await this.prismaMaster.equipment.deleteMany({
        where: { id: { in: ids } },
      });
      await this.activityLogs.log({
        userId: ctx.userId,
