...
export class ItemClassController {
    ...
    @Get()
    @Permissions('erp.item-class.read')
    async getAll() {
    ...
    @Post()
    @Permissions('erp.item-class.create')
    async createMany(@Body() dto: CreateItemClassesDto, @Req() req: any) {
        return this.itemClassService.createMany(dto.items, req.user.id || req.user.userId);
    }

    @Put('bulk')
    @Permissions('erp.item-class.update')
    ...
    @Delete('bulk')
    @Permissions('erp.item-class.delete')
