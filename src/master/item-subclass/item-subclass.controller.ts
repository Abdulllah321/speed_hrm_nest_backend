...
export class ItemSubclassController {
    ...
    @Get()
    @Permissions('erp.item-subclass.read')
    async getAll() {
    ...
    @Post()
    @Permissions('erp.item-subclass.create')
    async createMany(@Body() dto: CreateItemSubclassesDto, @Req() req: any) {
        return this.itemSubclassService.createMany(dto.items, req.user.id || req.user.userId);
    }

    @Put('bulk')
    @Permissions('erp.item-subclass.update')
    ...
    @Delete('bulk')
    @Permissions('erp.item-subclass.delete')
