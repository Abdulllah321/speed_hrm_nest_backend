...
import { CreateItemSubclassDto, UpdateItemSubclassDto, BulkUpdateItemSubclassItemDto } from './dto/item-subclass.dto';
...
export class ItemSubclassService {
    ...
    async getAll() {
        ...
        const userIds = [...new Set(subclasses.map((s) => s.createdById).filter(Boolean))];
        ...
        const data = subclasses.map((subclass) => {
            const creator = subclass.createdById ? userMap.get(subclass.createdById) : null;
            return {
                ...subclass,
    ...
    async createMany(items: CreateItemSubclassDto[], createdById: string) {
        try {
            ...
            return { status: true, data: result, message: 'Item subclasses created successfully' };
        } catch (error: any) {
    ...
    async update(
        ...
        try {
            const existing = await this.prismaMaster.itemSubclass.findUnique({ where: { id } });
            if (!existing) return { status: false, message: 'Item Subclass not found' };
            ...
            await this.cacheManager.del('item_subclasses_all');
            await this.cacheManager.del('item_classes_all');
            return { status: true, data: subclass, message: 'Item subclass updated successfully' };
        } catch (error: any) {
            return { status: false, message: error.message, data: null };
        }
    }
    ...
    async updateMany(
        ...
        try {
            ...
            await this.cacheManager.del('item_subclasses_all');
            await this.cacheManager.del('item_classes_all');
            return { status: true, data: updated, message: 'Item subclasses updated successfully' };
        } catch (error: any) {
            return { status: false, message: error.message, data: null };
        }
    }
    ...
    async deleteMany(
        ...
        try {
            ...
            await this.cacheManager.del('item_subclasses_all');
            await this.cacheManager.del('item_classes_all');
            return { status: true, data: result, message: 'Item subclasses deleted successfully' };
        } catch (error: any) {
            return { status: false, message: error.message, data: null };
        }
    }
    ...
    async delete(
        ...
        try {
            const existing = await this.prismaMaster.itemSubclass.findUnique({ where: { id } });
            const result = await this.prismaMaster.itemSubclass.delete({ where: { id } });
            ...
            await this.cacheManager.del('item_subclasses_all');
            await this.cacheManager.del('item_classes_all');
            return { status: true, data: result, message: 'Item subclass deleted successfully' };
        } catch (error: any) {
            return { status: false, message: error.message, data: null };
        }
    }
