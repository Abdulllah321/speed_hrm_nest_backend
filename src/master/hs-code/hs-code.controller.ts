...
export class HsCodeController {
    ...
    @Post('hs-codes')
    @UseGuards(JwtAuthGuard, PermissionGuard('hs-code.create'))
    create(@Body() dto: CreateHsCodeDto) {
        return this.hsCodeService.create(dto);
    }

    @Get('hs-codes')
    @UseGuards(JwtAuthGuard, PermissionGuard('hs-code.read'))
    list() {
    ...
    @Put('hs-codes/:id')
    @UseGuards(JwtAuthGuard, PermissionGuard('hs-code.update'))
    update(@Param('id') id: string, @Body() dto: UpdateHsCodeDto) {
        return this.hsCodeService.update(id, dto);
    }

    @Delete('hs-codes/:id')
    @UseGuards(JwtAuthGuard, PermissionGuard('hs-code.delete'))
