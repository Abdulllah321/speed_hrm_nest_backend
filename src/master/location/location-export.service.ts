...
export interface QueueLocationExportOptions {
  userId: string;
  search?: string;
  status?: string;
  isOnline?: string;
...
export class LocationExportService {
  private readonly logger = new Logger(LocationExportService.name);

  constructor(
    @InjectQueue('location-export') private readonly exportQueue: Queue,
    ...
    await this.exportQueue.add(
      {
        ...
        isStockLocation: opts.isStockLocation,
      },
      {
        jobId,
        attempts: 1,
  ...
  async streamExportFile(jobId: string, res: any): Promise<void> {
    ...
    const filename = `locations-export-${timestamp}.xlsx`;
