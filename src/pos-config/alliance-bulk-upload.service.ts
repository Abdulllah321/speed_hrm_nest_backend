...
@Injectable()
export class AllianceBulkUploadService {
    private readonly logger = new Logger(AllianceBulkUploadService.name);

    constructor(
        @InjectQueue('alliance-upload') private uploadQueue: Queue,
    ...
    async initiateValidation(
        ...
        const uploadDir = path.join(process.cwd(), 'uploads', 'bulk', 'alliance');
        if (!fs.existsSync(uploadDir)) {
            fs.mkdirSync(uploadDir, { recursive: true });
        }
        const ext = filename.split('.').pop();
        const filePath = path.join(uploadDir, `alliance-upload-${upload.id}.${ext}`);
        ...
        this.logger.log(`Alliance validation initiated: ${upload.id} (Job: ${job.id}), saved to ${filePath}`);
    ...
    async confirmUpload(uploadId: string, userId: string): Promise<{ uploadId: string; jobId: string }> {
        ...
        this.logger.log(`Alliance import confirmed: ${upload.id} (Job: ${job.id})`);
    ...
    async cancelUpload(uploadId: string): Promise<void> {
        ...
        this.logger.log(`Alliance upload cancelled: ${uploadId}`);
    }
