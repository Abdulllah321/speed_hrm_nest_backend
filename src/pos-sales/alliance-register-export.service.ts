...
export interface QueueAllianceRegisterExportOptions {
  userId: string;
  locationId?: string;
  startDate?: string;
  endDate?: string;
...
@Injectable()
export class AllianceRegisterExportService {
  ...
  async queueExport(opts: QueueAllianceRegisterExportOptions): Promise<{ jobId: string }> {
    ...
    await this.prisma.exportHistory.create({
      data: {
        id: jobId,
        userId: opts.userId,
        fileName: `alliance-register-${new Date().toISOString().slice(0, 10)}.${ext}`,
        filePath: path.join('uploads', 'exports', `export-${jobId}.${ext}`),
        moduleName: 'ALLIANCE_REGISTER_REPORT',
        status: 'PENDING',
      },
    });
